...
/**
 * Main application class for the Best Application.
...
@EnableJpaRepositories(
        basePackages = "com.spring.best.erp.repositories"
)
@Slf4j
public class BestApplication {
    ...
    public static void main(String[] args) {
        SpringApplication app = new SpringApplication(BestApplication.class);
        Environment env = app.run(args).getEnvironment();
        String host = getHostAddress();
        String port = env.getProperty("server.port", "9090");
        String url = "http";
        String contextPath = env.getProperty("server.servlet.context-path", "/Best/api/v1") + "/swagger-ui.html";
        String applicationUrl = String.format(url+"://%s:%s%s", host, port, contextPath);
        log.info("BestApplication started at: {}", applicationUrl);
    }
